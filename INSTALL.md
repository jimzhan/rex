# Rex: AI-Driven Project Scaffolding Guide

> **AI Execution Directive**: Follow these instructions strictly when a user asks you to scaffold or integrate a new project with `OpenCode`. Your primary mandate is **non-destructive integration**: never delete, overwrite, or alter existing product code, tests, or package files without explicit, confirmed user consent.
...
## 2. Goal
Intelligently inject the `rex` governance layer (`OpenCode` configurations) into the user's current working directory while preserving 100% of their existing project conventions, code, and documentation.
...
git clone --depth 1 https://github.com/jimzhan/rex.git rex
```
...
5. **Conflict Resolution**: If `opencode.json`, `skills-lock.json`, or `.opencode/` already exist, **pause execution**. Compare the template version/structure with the target version. Present the differences to the user and **ask for explicit approval** before replacing, merging, or restructuring anything.
...
## 8. Cleanup
Once the user confirms the installation is successful, delete the temporary cloned repository (`rex`) to maintain a clean environment.
